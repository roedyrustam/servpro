...
class Language_web_model extends CI_Model {
	...
	public function keyword_model()
	    ...
	    { 
	        ...
	        $data['lang_key'] = 'lg_'.str_replace(array(' ','!','&'),'_',strtolower($field_name));
	        $data['language'] = 'en';
...

	        {
	        	...
	        	$datas['lang_key'] = 'lg_'.str_replace(array(' ','!','&'),'_',strtolower($field_name));
    ...
    public function lang_manage_data($lan)
    {
      $query = $this->db->query(" SELECT * FROM language_management where page_key='subscriptions' AND language = '$lan' ");
      return $query->result_array();
    }
}
