...
class Requester_chat extends CI_Controller {

	 public function __construct() {
        parent::__construct();
        $this->load->model('requester_chat_model','chat');
	...
	public function get_requester_chat_history()
	{
		...
		$chat['last_record'] = $this->chat->request_last_record();
  ...
  // 		$this->load->vars($this->data);
	...
	public function chat_history()
	{
			...
			$this->data['page'] = 'requester-chat-history';
			$this->data['model'] = 'chat';
...
/*
		...
		if(!empty($this->input->post())){
			...
			if(count($history['chat_list']) >0){
				...
				foreach ($history['chat_list'] as $data) {
					...
					$time = utc_date_conversion($data['chat_utc_time']);
		...
		if(count($ios_target) > 0){
			...
			$error1 = 1;
			 ...
			 foreach ($history as $mychat) {
			 	$time = utc_date_conversion($mychat['chat_utc_time']);
	...
	public function requester_unread_count()
	{
		$to=$this->input->post('to');

		$result=$this->db->where('chat_to',$to)->where('status',0)->get('chats_requester')->num_rows();
		echo $result;
	}

	public function requester_readchat()
	{
		$from=$this->input->post('from');
		$to=$this->input->post('to');
